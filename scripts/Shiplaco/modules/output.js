...
class OutputWindow {
  ...
  static init() {
    ...
    this.windowElement.id = 'plasheek-output-window';
    this.windowElement.innerHTML = `
      <div id="plasheek-output-header" style="cursor: move; background: ${styles.headerBg}; color: ${styles.headerColor}; padding: 5px; font-size: 12px; display: flex; justify-content: space-between; align-items: center; height: 30px; flex-shrink: 0;">
        <div style="display: flex; align-items: center; gap: 5px;">
          <span>${Localization.get('outputWindowTitle')}</span>
          <select id="plasheek-message-filter" style="font-size: 10px; padding: 2px; background: ${styles.settingsInputBg}; color: ${styles.settingsInputText}; border: 1px solid ${styles.settingsBorder}; border-radius: 3px;">
        ...
        <div style="display: flex; align-items: center; gap: 5px;">
          <button id="plasheek-output-clear" style="background: ${styles.buttonBg}; color: ${styles.buttonColor}; border: 1px solid ${styles.settingsBorder}; cursor: pointer; font-size: 10px; padding: 2px 5px; border-radius: 3px;">${Localization.get('outputClearButton')}</button>
          <button id="plasheek-output-close" style="background: ${styles.buttonBg}; color: ${styles.buttonColor}; border: none; cursor: pointer; font-size: 16px; width: 20px; height: 20px; border-radius: 50%; display: flex; align-items: center; justify-content: center;">X</button>
        </div>
      </div>
      <div id="plasheek-output-content" style="height: 365px; overflow-y: auto; padding: 5px; font-size: 12px; color: ${styles.contentColor}; max-height: 365px;"></div>
      <div id="plasheek-output-resize" style="width: 15px; height: 15px; cursor: nwse-resize; position: absolute; bottom: 0; right: 0; background: ${styles.windowBorder}; z-index: 1;"></div>
    `;
    ...
    this.contentElement = document.getElementById('plasheek-output-content');

    // Обработчики событий для перемещения и изменения размера
    const header = document.getElementById('plasheek-output-header');
    const resizeHandle = document.getElementById('plasheek-output-resize');
    const closeButton = document.getElementById('plasheek-output-close');
    const clearButton = document.getElementById('plasheek-output-clear');
    const filterSelect = document.getElementById('plasheek-message-filter');
  ...
  static show() {
    ...
    const content = document.getElementById('plasheek-output-content');
    if (content) {
