...
(function () {
  ...
  async function preloadCriticalTranslations(cache) {
    ...
    await Promise.allSettled(
      criticalTerms.map((term) => cache.getCachedTranslation(term)),
    );
  }
            ...
            acceptNode: (node) => {
              ...
              if (node.classList && window.IGNORED_CLASSES) {
                for (const cls of node.classList) {
                  if (
                    window.IGNORED_CLASSES.has(cls) &&
                    !window.ALWAYS_TRANSLATE_CLASSES.has(cls)
                  ) {
        ...
        window.simpleTranslateFallback = (node) => {
          if (node.nodeType === Node.TEXT_NODE) {
            ...
            for (const child of node.childNodes)
              window.simpleTranslateFallback(child);
          }
        };
