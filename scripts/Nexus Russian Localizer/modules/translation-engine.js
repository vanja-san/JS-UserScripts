...
// 1. Dynamic templates
const dynamicResult = await this.applyDynamicTemplates(originalText, parent);
...
this.#processedTextNodes.add(node);
await this.#cache.cacheTranslation(text, contextual.context, contextual.translation);
return true;
}
}
...
if (!translated) {
const contextual = this.#contextMatcher?.findTranslation(value, element);
...
(([rect.top](https://rect.top) < window.innerHeight && rect.bottom > 0) ||
node === document.body);
...
subtree: true,
attributes: true,
attributeFilter: ['title', 'placeholder', 'alt', 'data-tooltip', 'aria-label', 'value'],
characterData: true
};
