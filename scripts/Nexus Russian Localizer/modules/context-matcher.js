...
class ContextMatcher {
  ...
  buildRulesCache() {
    ...
    for (const [text, contexts] of Object.entries(
      this.#translations.contextual,
    )) {
      if (typeof text !== "string" || text.length > 500) continue;
      const rules = Object.entries(contexts)
        .map(([context, translation]) => {
  ...
  findTranslation(text, element) {
    if (!text || !element || typeof text !== "string" || text.length > 2000)
      return null;
